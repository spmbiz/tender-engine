...
def main():
 ...
 winners=con.execute("""WITH s AS (
 SELECT service_family,service_subtype,native_code,currency,supplier,supplier_shape,count(*) awards
 FROM z WHERE supplier<>''
 GROUP BY service_family,service_subtype,native_code,currency,supplier,supplier_shape)
 SELECT *,awards*1.0/sum(awards) over(partition by service_family,service_subtype,native_code,currency) supplier_share
 FROM s
 QUALIFY row_number() over(partition by service_family,service_subtype,native_code,currency order by awards desc,supplier)<=20
 ORDER BY service_family,service_subtype,awards DESC""").fetchall();write(out/'top_winners.csv',['family','subtype','native_code','currency','supplier','supplier_shape','awards','supplier_share'],winners)
