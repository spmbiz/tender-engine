...
def main():
    ...
    """)
    # Primary family is broad. Subfamily is designed to expose economic model rather than force SPM fit.
    con.execute("""
      CREATE TABLE c0 AS
      SELECT *,CASE
    ...
    con.execute("""
      ...
      SELECT *,CASE
        ...
        ELSE NULL END subfamily
      FROM c0 WHERE market_family IS NOT NULL
    """)
    matrix=con.execute("""
      WITH b AS (
        SELECT market_family,subfamily,currency,count(*) records,count(distinct nullif(buyer,'')) buyers,count(distinct nullif(supplier,'')) suppliers,
      ...
      ss0 AS (SELECT market_family,subfamily,currency,supplier,count(*) n FROM y WHERE supplier<>'' GROUP BY 1,2,3,4),
      ss AS (SELECT market_family,subfamily,currency,max(n)*1.0/sum(n) top_supplier_share FROM ss0 GROUP BY 1,2,3)
      SELECT b.*,coalesce(rb.repeat_buyers,0),ss.top_supplier_share
      FROM b LEFT JOIN rb USING(market_family,subfamily,currency) LEFT JOIN ss USING(market_family,subfamily,currency)
      ORDER BY market_family,records DESC
    """).fetchall()
    write(out/'market_matrix.csv',['family','subfamily','currency','records','buyers','suppliers','p25_value','median_value','p75_value','repeat_buyers','top_supplier_share'],matrix)
    winners=con.execute("""
      SELECT market_family,subfamily,currency,supplier,count(*) awards,count(*)*1.0/sum(count(*)) over(partition by market_family,subfamily,currency) supplier_share
      FROM y WHERE supplier<>'' GROUP BY 1,2,3,4
      QUALIFY row_number() over(partition by market_family,subfamily,currency order by count(*) desc,supplier)<=15
      ORDER BY market_family,subfamily,awards DESC
    """).fetchall();write(out/'top_winners.csv',['family','subfamily','currency','supplier','awards','supplier_share'],winners)
    buyers=con.execute("""
      SELECT market_family,subfamily,currency,buyer,count(*) awards FROM y WHERE buyer<>'' GROUP BY 1,2,3,4
      QUALIFY row_number() over(partition by market_family,subfamily,currency order by count(*) desc,buyer)<=15
      ORDER BY market_family,subfamily,awards DESC
    """).fetchall();write(out/'top_buyers.csv',['family','subfamily','currency','buyer','awards'],buyers)
    examples=con.execute("""
      SELECT market_family,subfamily,currency,tender_id,buyer,supplier,title,award_value,subcategory,category FROM y
      QUALIFY row_number() over(partition by market_family,subfamily,currency order by award_value desc nulls last,tender_id)<=12
      ORDER BY market_family,subfamily,award_value DESC NULLS LAST
    """).fetchall();write(out/'examples.csv',['family','subfamily','currency','tender_id','buyer','supplier','title','award_value','subcategory','category'],examples)
    summary={'version':'HISTORICAL_AU_ASYMMETRIC_SERVICES_V1','archive_records_scanned':con.execute('select count(*) from x').fetchone()[0],'classified_rows':con.execute('select count(*) from y').fetchone()[0],'families':dict(con.execute('select market_family,count(*) from y group by 1 order by 2 desc').fetchall()),'subfamilies':dict(con.execute('select subfamily,count(*) from y group by 1 order by 2 desc').fetchall()),'grain':'AWARD_FIRST_PROCUREMENT','historical_only':True,'live_used':False,'dce_used':False,'warning':'Semantic/economic decomposition only. Award-first evidence does not establish current panel access or bidability.'}
    (out/'summary.json').write_text(json.dumps(summary,indent=2,ensure_ascii=False),encoding='utf-8')
    lines=['# Australia Asymmetric Services Decomposition v1','',f"- AusTender records scanned: **{summary['archive_records_scanned']:,}**",f"- classified rows: **{summary['classified_rows']:,}**",'', 'Award-first historical evidence only. This report decomposes economic delivery models; it does not prove current access, licensing or panel eligibility.','']
    for r in matrix:
