...
def main():
    ...
    matrix=con.execute("""
    WITH base AS (
      SELECT market_family,native_code,native_subcategory,currency,count(*) records,count(distinct nullif(buyer,'')) buyers,count(distinct nullif(supplier,'')) suppliers,
    ...
    ), rb0 AS (SELECT market_family,native_code,native_subcategory,currency,buyer,count(*) n FROM y WHERE buyer<>'' GROUP BY 1,2,3,4,5),
    rb AS (SELECT market_family,native_code,native_subcategory,currency,count(*) FILTER(WHERE n>=2) repeat_buyers FROM rb0 GROUP BY 1,2,3,4),
    ss0 AS (SELECT market_family,native_code,native_subcategory,currency,supplier,count(*) n FROM y WHERE supplier<>'' GROUP BY 1,2,3,4,5),
    ss AS (SELECT market_family,native_code,native_subcategory,currency,max(n)*1.0/sum(n) top_supplier_share FROM ss0 GROUP BY 1,2,3,4)
    SELECT b.*,coalesce(rb.repeat_buyers,0),ss.top_supplier_share
    FROM base b LEFT JOIN rb USING(market_family,native_code,native_subcategory,currency)
    LEFT JOIN ss USING(market_family,native_code,native_subcategory,currency)
    ...
    winners=con.execute("""
      SELECT market_family,native_code,native_subcategory,currency,supplier,count(*) awards,
             count(*)*1.0/sum(count(*)) over(partition by market_family,native_code,native_subcategory,currency) supplier_share
      FROM y WHERE supplier<>'' GROUP BY 1,2,3,4,5
      QUALIFY row_number() over(partition by market_family,native_code,native_subcategory,currency order by count(*) desc,supplier)<=10
      ORDER BY market_family,native_code,native_subcategory,awards DESC
    """).fetchall()
    write(out/'top_winners.csv',['family','native_code','native_subcategory','currency','supplier','awards','supplier_share'],winners)
    buyers=con.execute("""
      SELECT market_family,native_code,native_subcategory,currency,buyer,count(*) awards
      FROM y WHERE buyer<>'' GROUP BY 1,2,3,4,5
      QUALIFY row_number() over(partition by market_family,native_code,native_subcategory,currency order by count(*) desc,buyer)<=10
      ORDER BY market_family,native_code,native_subcategory,awards DESC
    """).fetchall()
    write(out/'top_buyers.csv',['family','native_code','native_subcategory','currency','buyer','awards'],buyers)
    examples=con.execute("""
      SELECT market_family,native_code,native_subcategory,currency,tender_id,buyer,supplier,title,award_value
      FROM y QUALIFY row_number() over(partition by market_family,native_code,native_subcategory,currency order by award_value desc nulls last,tender_id)<=5
      ORDER BY market_family,native_code,native_subcategory,award_value DESC NULLS LAST
    """).fetchall()
