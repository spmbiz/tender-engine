...
S=requests.Session(); S.headers.update({'User-Agent':'Tender-Engine/4.4 (+public procurement research)','Accept':'text/csv,*/*'})
...
def main():
    ...
    rows=[]; headers=reader.fieldnames or []
    for row in reader:
        nid=first(row,'NoticeId','Notice ID','notice_id','Id')
        sol=first(row,'SolicitationNumber','Solicitation Number','Solicitation #')
        title=first(row,'Title','Opportunity Title','Description')
        if not (nid or sol or title):continue
        deadline=pdt(first(row,'ResponseDeadLine','Response Date','ResponseDate','ArchiveDate','Archive Date'))
        posted=pdt(first(row,'PostedDate','Posted Date','Publish Date'))
        active=first(row,'Active','Status','Archive Type')
    ...
    stats={'source':'US_SAM_BULK','raw_materialized':len(rows),'current_materialized':len(current),'generated_at':NOW.isoformat(),'errors':[],'official_url':URL,'download_bytes':len(r.content),'headers':headers}
