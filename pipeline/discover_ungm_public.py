...
def persist(records, *, pages, exhausted, errors, warnings, telemetry):
    ...
    stats = {
        "source": "UNGM_PUBLIC",
        "portal": "UNGM",
        "listing_contract": "UNGM_PUBLIC_ACTIVE_OPPORTUNITIES_V1",
        ...
        "source_url": URL,
        "semantics": "UNGM public Procurement Opportunities is read with Only currently active enabled. Coverage credit requires exhaustive pagination of the public result set; no authenticated UNGM API is assumed.",
    }
...
async def ensure_active_only(page, telemetry):
    # UNGM documents active opportunities as the default. We still explicitly
    # keep the checkbox checked where the DOM exposes it.
    labels = page.locator("label")
    for i in range(await labels.count()):
        label = labels.nth(i)
        text = clean(await label.inner_text())
        if "only currently active" not in text.lower():
            continue
        try:
            ...
            telemetry["active_filter_explicit"] = True
            return
        except Exception:
            pass
    # Fallback: inspect checkbox parents for the accessible text.
...
async def click_search(page):
    for loc in (
        page.get_by_role("button", name=re.compile(r"^search\s*$", re.I)),
...
async def table_headers(tr):
    cells = tr.locator("th")
    return [clean(await cells.nth(i).inner_text()).lower() for i in range(await cells.count())]


async def parse_page(page, records, telemetry):
    links = page.locator("a[href*='/Public/Notice/']")
    found = 0
    seen_on_page = set()
    for i in range(await links.count()):
        a = links.nth(i)
        href = clean(await a.get_attribute("href"))
        match = NOTICE_RE.search(href)
        if not match:
            continue
        notice_id = match.group(1)
        if notice_id in seen_on_page:
            continue
        seen_on_page.add(notice_id)
        ...
        try:
            ...
            if await row.count():
                tds = row.locator("td")
                cells = [clean(await tds.nth(j).inner_text()) for j in range(await tds.count())]
        except Exception:
            text, cells = title, []

        # The public grid columns are Title, Deadline, Published, Organization,
        # Type, Reference and beneficiary country. Prefer cell positions, then
        # fall back to labelled/date extraction without inventing missing fields.
        deadline = None
        published = None
        organization = None
        ...
        country = None
        if len(cells) >= 2:
            deadline = parse_date(cells[1])
        if len(cells) >= 3:
            published = parse_date(cells[2])
        if len(cells) >= 4:
            organization = cells[3] or None
        if len(cells) >= 5:
            opportunity_type = cells[4] or None
        if len(cells) >= 6:
            reference = cells[5] or None
        if len(cells) >= 7:
            country = cells[6] or None
        if deadline is None:
            dm = re.search(r"Deadline(?:\s+on)?\s*:?\s*([^|]{6,40})", text, re.I)
            if dm:
                m = DATE_RE.search(dm.group(1))
                if m:
        ...
        records[cid] = {
            ...
            "current": current,
            "currentness_evidence": "UNGM_PUBLIC_ACTIVE_FILTER" if deadline is None else "UNGM_ACTIVE_FILTER_PLUS_DEADLINE",
            "notice_type": opportunity_type,
            "reference": reference,
            "notice_url": urljoin(URL, href),
            "description": text,
            "route": {"notice_id": notice_id},
            "discovered_at": NOW.isoformat(),
        }
        found += 1
    telemetry.setdefault("pages", []).append({"page_index": len(telemetry.get("pages", [])) + 1, "rows_parsed": found})
    return found
    ...
    try:
        async with async_playwright() as pw:
            ...
            if response and response.status >= 400:
                raise RuntimeError(f"UNGM public search returned HTTP {response.status}")
            await page.wait_for_timeout(WAIT_MS)
            await ensure_active_only(page, telemetry)
            await click_search(page)

            # UNGM is JS-backed; wait for public notice result links rather than
            # assuming the initial HTML contains the result grid.
            try:
                await page.locator("a[href*='/Public/Notice/']").first.wait_for(timeout=30000)
            except Exception:
                body = clean(await page.locator("body").inner_text())
                if "captcha" in body.lower():
                    errors.append({"type": "PUBLIC_SEARCH_CAPTCHA_GATE"})
                else:
                    errors.append({"type": "PUBLIC_RESULT_GRID_NOT_MATERIALIZED"})
            ...
            for _ in range(MAX_PAGES):
                ...
                for i in range(await loc.count()):
                    href = clean(await loc.nth(i).get_attribute("href"))
                    m = NOTICE_RE.search(href)
                    if m:
                        ids.append(m.group(1))
                signature = tuple(sorted(set(ids)))
                if not signature:
                    errors.append({"type": "ZERO_NOTICE_LINKS_ON_ACTIVE_RESULTS"})
                    break
                if signature in seen_signatures:
                    errors.append({"type": "PAGINATION_REPEATED_RESULT_SET", "page": pages + 1})
                    break
                seen_signatures.add(signature)
                await parse_page(page, records, telemetry)
                pages += 1
                persist(records, pages=pages, exhausted=False, errors=errors, warnings=warnings, telemetry=telemetry)
        ...
        errors.append({"type": "UNGM_BROWSER_ERROR", "error": repr(exc)})

    if not records and not errors:
        errors.append({"type": "ZERO_ACTIVE_NOTICES"})
    stats = persist(records, pages=pages, exhausted=exhausted, errors=errors, warnings=warnings, telemetry=telemetry)
