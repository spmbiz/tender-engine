...
def main():
    ...
    portal_attempt_status = defaultdict(Counter)
    ...
    for mp in sorted(root.rglob("manifest.json")):
        ...
        for item in list(attempts) + list(unsupported):
            ...
            attempt_status = str(item.get("status") or ("UNSUPPORTED" if item in unsupported else "UNKNOWN"))
            portal_attempt_status[portal][attempt_status] += 1
    ...
    result = {
        ...
        "domain_status_counts": {
            host: dict(counter.most_common())
            for host, counter in sorted(domain_status.items(), key=lambda kv: (-sum(kv[1].values()), kv[0]))
        },
        "portal_attempt_status_counts": {
            portal: dict(counter.most_common())
            for portal, counter in sorted(portal_attempt_status.items(), key=lambda kv: (-sum(kv[1].values()), kv[0]))
        },
