...
S=requests.Session();S.headers.update({'User-Agent':'Tender-Engine/5.2 (+public procurement research)','Accept':'application/json,*/*'})
...
def is_json_resource(r:dict):
    fmt=str(r.get('format') or '').lower();url=str(r.get('url') or '').lower();name=str(r.get('name') or '').lower()
...
def fetch_package(year:int):
    ...
    try:
        ...
        rec={'year':year,'package_id':pid,'url':r.url,'status':r.status_code,'bytes':len(r.content),'content_type':r.headers.get('content-type'),'preview':r.text[:250] if len(r.content)<5000 else None}
...
def download_resource(rsrc:dict,idx:int):
    ...
    try:
        with S.get(url,timeout=180,stream=True,allow_redirects=True) as resp:
            ...
            if meta['content_length'] and meta['content_length']>DOWNLOAD_MAX:meta['skipped']='CONTENT_LENGTH_OVER_CAP';return meta
            total=0
        ...
        try:data=json.loads(path.read_bytes());meta['release_count']=count_releases(data)
...
def filesystem_probe():
    inv=[]
    # Search current years first. Search-engine indexed 2025 resource pages still use this official filesystem shape.
    for y in range(NOW.year,2016,-1):
        for m in range(12,0,-1):
            if y==NOW.year and m>NOW.month:continue
            url=LEGACY.format(year=y,month=m)
            try:
                r=S.get(url,timeout=45,stream=True)
                rec={'year':y,'month':m,'url':url,'status':r.status_code,'content_length':int(r.headers.get('content-length') or 0),'content_type':r.headers.get('content-type')};inv.append(rec)
            except Exception as exc:inv.append({'year':y,'month':m,'url':url,'error':repr(exc)});continue
            if r.status_code==200:return inv,rec
    return inv,None

def main():
    years=list(range(NOW.year,NOW.year-YEARS,-1));package_inventory=[];resources=[]
    for y in years:
        rec,pkg=fetch_package(y);package_inventory.append(rec)
        if not pkg:continue
        for r in pkg.get('resources') or []:
            if not isinstance(r,dict) or not is_json_resource(r):continue
            rr={k:r.get(k) for k in ('id','name','format','url','created','last_modified','size','description')};rr['package_year']=y;resources.append(rr)
    resources.sort(key=resource_rank,reverse=True)
    fallback_inventory=[];fallback_hit=None
    if not resources:fallback_inventory,fallback_hit=filesystem_probe()
    materialize=list(resources)
    if not materialize and fallback_hit:
        materialize=[{'id':f"filesystem-{fallback_hit['year']}-{fallback_hit['month']:02d}",'name':f"ocds_appalti_ordinari_{fallback_hit['year']}_{fallback_hit['month']:02d}.json",'format':'JSON','url':fallback_hit['url'],'package_year':fallback_hit['year']}]
    downloaded=[]
    for i,r in enumerate(materialize[:DOWNLOAD_RESOURCES]):downloaded.append(download_resource(r,i))
    ...
    available=bool(materialize)
    stats={'source':'IT_ANAC_OCDS_BULK','raw_materialized':sum(x.get('release_count') or 0 for x in downloaded),'current_materialized':0,'lane':'ARCHIVE_OCDS_MONTHLY_BULK','generated_at':NOW.isoformat(),'errors':[] if available else [{'type':'NO_AVAILABLE_OCDS_RESOURCE'}],'official_ckan_api':CKAN,'package_pattern':'ocds-appalti-ordinari-{year}','filesystem_pattern':LEGACY,'package_inventory_count':len(package_inventory),'json_resource_count':len(resources),'latest_resource':resources[0] if resources else fallback_hit,'downloaded':downloaded,'download_cap_bytes':DOWNLOAD_MAX,'download_resources_requested':DOWNLOAD_RESOURCES,'semantics':'Archive evidence lane. Current live competition discovery remains separate; do not treat these OCDS packages as a live-open-opportunity feed.'}
