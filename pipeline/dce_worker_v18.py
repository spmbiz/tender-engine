...
def _rendered_de_routes_v18(url: str, out: Path, manifest: dict) -> list[tuple[int, str, str]]:
    ...
    try:
        ...
        # Keep the existing anchor-based signal, but add visible-text URLs because
        # the German Publication Service often renders the supplier-platform address
        # as text under the procurement-document field instead of a clickable anchor.
        try:
        ...
        for item in anchors or []:
            ...
            score = 0
            if FILE_RE.search(href): score += 12
            if v14.portal_for_url_v14(href): score += 10
            if host and host != page_host and PROCUREMENT_HOST_RE.search(host): score += 8
            if PROCUREMENT_CONTEXT_RE.search(f"{text} {href}"): score += 8
            if score:
                prev = found.get(href)
        ...
        for match in URL_RE.finditer(body or ""):
            ...
            if FILE_RE.search(href): score += 12
            if classified: score += 12
            if PROCUREMENT_HOST_RE.search(host): score += 8
            if PROCUREMENT_CONTEXT_RE.search(context_text): score += 10
            # External links need at least two procurement signals, or a known
            # downstream family, before they can become a resolver route.
            if score < 12:
                continue
            prev = found.get(href)
            if prev is None or score > prev[0]:
...
# v7.adapter_germany_doe resolves this helper at call time. Replace only the
# discovery helper; the mature v7 download/page cascade and barrier semantics stay
# unchanged.
