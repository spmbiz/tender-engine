...
def scrape_rows(page,page_no):
    data=page.locator("table tbody tr").evaluate_all("""rows => rows.map(r => ({cells:[...r.querySelectorAll('td')].map(x => (x.innerText||'').trim()),links:[...r.querySelectorAll('a[href]')].map(a => ({text:(a.innerText||'').trim(),href:a.href}))}))""")
    out=[]
    for item in data:
        cells=[clean(x) for x in item.get("cells",[])]
        if len(cells)<8:continue
        buyer,title,internal_id=cells[0],cells[1],cells[2]
        procedure=cells[3] if len(cells)>3 else None;pjn=cells[5] if len(cells)>5 else None
        deadline=parse_deadline(cells[7] if len(cells)>7 else "");status=cells[9] if len(cells)>9 else (cells[-1] if cells else "")
        ...
        links=item.get("links") or [];detail=None
        for a in links:
            txt=clean(a.get("text"));href=a.get("href")
            if href and (txt in {title,internal_id,pjn} or (title and txt and txt in title)):detail=href;break
        ...
        out.append({"candidate_id":f"SI-EJN:{identity}","source":"SI_EJN","portal":"SI_EJN","notice_id":identity,"title":title,"buyer":buyer or None,"deadline":deadline.isoformat() if deadline else None,"current":True,"notice_url":detail or URL,"description":" | ".join(cells),"procurement_method":procedure or None,"currency":"EUR","route":{"detail_url":detail or URL,"public_url":detail or URL,"internal_id":internal_id,"pjn_id":pjn},"discovery_page":page_no,"discovered_at":NOW.isoformat()})
    return out
...
def main():
    ...
    with sync_playwright() as p:
        executable=system_chrome()
        if not executable:
            raise SystemExit("No system Chrome/Chromium found; workflow must install a browser before Slovenia discovery")
        ...
        for pg in range(1,MAX_PAGES+1):
            try:
                ...
                before=page.locator("table tbody tr").first.inner_text(timeout=5000);nxt.first.click(timeout=10000);page.wait_for_timeout(650)
                try:page.wait_for_function("prev => {const r=document.querySelector('table tbody tr'); return r && (r.innerText||'') !== prev}",arg=before,timeout=10000)
                except Exception:page.wait_for_timeout(1000)
            except Exception as exc:errors.append({"page":pg,"error":repr(exc)});break
        browser.close()
    ...
    stats={"source":"SI_EJN","official_url":URL,"pages_fetched":pages,"current_materialized":len(rows),"errors":errors,"chrome":executable,"generated_at":NOW.isoformat()}
