...
def compact_green(rec: dict, run_id: str, shard: str, published_at: str) -> dict:
    ...
    return {
        ...
        "authority_conflicts": rec.get("authority_conflicts") or {},
...
def merge_green(existing: dict, incoming: list[dict], run_id: str, shard: str, max_green: int) -> dict:
    ...
    for rec in incoming:
        ...
        if current is None or green_sort_key(rec) >= green_sort_key(current):
            merged[key] = rec

    items = sorted(merged.values(), key=green_sort_key, reverse=True)
...
def _deadline_info(rec: dict) -> tuple[dict, str, bool]:
    ...
    resolved = status in RESOLVED_DEADLINE and not bool(deadline.get("conflict"))
    return deadline, status, resolved


def compact_review(rec: dict, run_id: str, shard: str, published_at: str) -> dict:
    evidence = rec.get("gate_evidence_candidates") or {}
    coverage = sum(1 for items in evidence.values() if isinstance(items, list) and items)
    deadline, deadline_status, deadline_resolved = _deadline_info(rec)
    return {
        "candidate_id": rec.get("candidate_id"),
        "title": rec.get("title"),
        "buyer": rec.get("buyer"),
        ...
        "estimated_value": rec.get("estimated_value"),
        "currency": rec.get("currency"),
        "preliminary_score": rec.get("preliminary_score"),
        "priority_score": int(rec.get("final_score") or 0),
        "content_quality": rec.get("content_quality"),
        "gate_readiness": bool(rec.get("gate_readiness")),
        ...
        "source_dce_run_id": int(run_id) if str(run_id).isdigit() else run_id,
        "source_shard": int(shard) if str(shard).isdigit() else shard,
        "hot_ready_at": published_at,
        "review_contract": "Gate-ready authoritative DCE evidence pack. ChatGPT must use only supplied evidence plus explicitly known bidder facts; missing bidder facts stay UNKNOWN. FINAL_SUPER_GREEN requires every mandatory gate resolved and authoritative deadline reconciliation.",
    }


def _deadline_open(rec: dict) -> bool:
    raw = str(rec.get("deadline") or "")[:10]
    try:
...
def merge_review(existing: dict, incoming: list[dict], resolved_keys: set[str], run_id: str, shard: str, max_items: int) -> dict:
    ...
    for rec in existing.get("items") or []:
        if isinstance(rec, dict) and item_key(rec) not in resolved_keys and _deadline_open(rec):
            merged[item_key(rec)] = rec
    for rec in incoming:
        if not _deadline_open(rec):
            continue
        key = item_key(rec)
        current = merged.get(key)
        if current is None or review_sort_key(rec) >= review_sort_key(current):
            merged[key] = rec
    ...
    return {
        ...
        "count": len(items),
        "items": items,
        "instruction": "Review highest-ranked items first. These are DCE evidence packs, not final verdicts. Unknown is never PASS. Apply the Tender mandatory-gate contract and deadline authority before returning FINAL_SUPER_GREEN.",
    }
...
def github_get(repo: str, path: str, branch: str, token: str) -> tuple[dict, str | None]:
    ...
    headers = {
        ...
        "User-Agent": "tender-hot-publisher/2.0",
    }
...
def publish_retry(repo: str, path: str, branch: str, token: str, builder, message: str, attempts: int):
    last_error = None
    for attempt in range(1, max(1, attempts) + 1):
        try:
            existing, sha = github_get(repo, path, branch, token)
            payload = builder(existing)
            r = github_put(repo, path, branch, token, payload, sha, message)
            if r.status_code in {200, 201}:
                return {"published": True, "attempt": attempt, "path": path, "payload": payload}
    ...
    raise RuntimeError(f"publish failed for {path}: {last_error}")
...
def main() -> None:
    ...
    ap.add_argument("--max-review", type=int, default=30)
    ...
    green_incoming = [
        compact_green(rec, args.run_id, args.shard, published_at)
        for rec in records
        if str(rec.get("classification") or "").upper() in GREEN_CLASSES
    ]
    review_incoming = [
        compact_review(rec, args.run_id, args.shard, published_at)
        for rec in records
        if str(rec.get("classification") or "").upper() == REVIEW_CLASS
        and rec.get("gate_readiness")
        and rec.get("gate_evidence_candidates")
    ]
    resolved_keys = {
        item_key(rec)
        for rec in records
    ...
    result = {
        "schema": "TENDER_HOT_PUBLISH_V2",
        "run_id": args.run_id,
        "shard": args.shard,
        "green_delta": len(green_incoming),
        "review_delta": len(review_incoming),
        "green": None,
        "gpt_review": None,
    }

    if green_incoming:
        result["green"] = publish_retry(
            args.repo,
            args.path,
            args.branch,
            token,
            lambda existing: merge_green(existing, green_incoming, args.run_id, args.shard, args.max_green),
            f"tender: hot green DCE {args.run_id} shard {args.shard}",
            args.attempts,
        )

    if review_incoming or resolved_keys:
        result["gpt_review"] = publish_retry(
            args.repo,
            args.review_path,
            args.branch,
            token,
            lambda existing: merge_review(existing, review_incoming, resolved_keys, args.run_id, args.shard, args.max_review),
            f"tender: GPT-ready DCE bank {args.run_id} shard {args.shard}",
            args.attempts,
        )

    Path(args.out).write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps({
        "published": bool(result["green"] or result["gpt_review"]),
        "green_delta": len(green_incoming),
        "review_delta": len(review_incoming),
        "green_path": args.path if result["green"] else None,
        "gpt_review_path": args.review_path if result["gpt_review"] else None,
    }, ensure_ascii=False))
