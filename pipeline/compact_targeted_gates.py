...
def main():
    ...
    for candidate in sorted(root.iterdir()):
        ...
        rec = {
            ...
            "files": [{"name":x.get("name"),"size":x.get("size"),"source":x.get("source") or x.get("url")} for x in (m.get("files") or []) if isinstance(x,dict)],
    ...
    print(json.dumps({"candidate_count":len(out),"out":args.out},indent=2))
