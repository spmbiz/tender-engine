...
def _repo_capacity_state(repo: str, ignore_run_id: str = "") -> tuple[Counter, Counter]:
    ...
    for run_status in ("in_progress", "queued"):
        ...
        for run in (data or {}).get("workflow_runs") or []:
            ...
            workload = _classify(
                repo,
                str(run.get("name") or run.get("display_title") or ""),
                str(run.get("path") or ""),
            )
...
def dynamic_tender_parallel(requested: int) -> tuple[int, dict]:
    ...
    # Queued sibling jobs already express demand but do not occupy a runner. We
    # still reserve enough free capacity to bring a demanding sibling to its fair
    # active target. This keeps fairness without pretending queued work is running.
    ...
    # Durable selection/controller backlog is the stronger demand signal for the
    # DCE lane. Remote broker demand may be stale, so never let a stale low number
    # cap the explicitly requested DCE parallelism below its fair active room.
