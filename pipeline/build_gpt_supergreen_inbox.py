...
def rank(r):
    ...
    return (action,int(r.get('gpt_priority_score') or 0),int(bool(r.get('gate_readiness'))),str(r.get('candidate_id') or ''))
def main():
    ...
    for r in existing.get('pending_final_review',[]) or []:
        if isinstance(r,dict) and key(r) and key(r) not in resolved and open_deadline(r): merged[key(r)]=r
    for r in load_jsonl(args.triage):
        if not key(r) or key(r) in resolved or not open_deadline(r): continue
        if r.get('recommended_gpt_action') in {'DROP_EXPIRED','DROP_OR_PARTNER'}: continue
    ...
    confirmed=[x for x in (final.get('items') or []) if isinstance(x,dict) and open_deadline(x)]
    confirmed.sort(key=lambda r:(str(r.get('classification') or '')=='FINAL_SUPER_GREEN',int(r.get('final_score') or 0)),reverse=True)
    payload={
      'schema':'GPT_INSTANT_SUPERGREEN_INBOX_V1','updated_at':datetime.now(timezone.utc).isoformat(),'latest_source_dce_run_id':int(args.source_run) if str(args.source_run).isdigit() else args.source_run,
      'confirmed_supergreens':confirmed,'pending_final_review':pending,
      'counts':{'confirmed_supergreens':sum(str(x.get('classification'))=='FINAL_SUPER_GREEN' for x in confirmed),'confirmed_green_total':len(confirmed),'pending_final_review':len(pending),'review_now':sum(x.get('recommended_gpt_action')=='FINAL_REVIEW_NOW' for x in pending)},
      'answer_contract':'When the user asks what supergreens exist, read this file first. Report confirmed_supergreens immediately. Then adjudicate pending_final_review rows marked FINAL_REVIEW_NOW from evidence_by_gate. Qwen is pre-read only; missing evidence is UNKNOWN. Persist a GPT final verdict only after mandatory gates and deadline authority are resolved.',
