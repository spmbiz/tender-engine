...
def _za_ocid(candidate: dict) -> str:
    ...
    # Older normalized candidates sometimes only retain the release-shaped ID.
    # Remove a terminal ISO date when present, but never invent an OCID from an
...
def _direct_candidate_docs(candidate: dict, out: Path, manifest: dict) -> bool:
    # The common OCDS normalizer already materializes tender/planning/contract
    # document URLs into route.document_urls. Try that cheapest path before making
...
def adapter_za_etenders_v16(candidate: dict, out: Path, manifest: dict) -> None:
    """Resolve South Africa eTenders documents through the official OCDS API.

    Discovery already uses the same public API and normalizes tender.documents,
    planning.documents and contract documents. This adapter keeps those candidates
    on the HTTP shard lane instead of spending browser capacity on an API record.
    """
    ...
    # Persist only lightweight API evidence outside the files/ DCE evidence area.
    try:
    ...
    if downloaded:
        ...
        # The authoritative public record advertised documents, but none validated
        # as a file. Treat this as retryable rather than falsely declaring no DCE.
