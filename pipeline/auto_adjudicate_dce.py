...
def number_evidence(row: dict):
    ...
    for gate in REQUIRED_GATES:
        ...
        for i,item in enumerate((snippets.get(gate) or [])[:12],1):
            ref=f"{gate}:{i}"
            if isinstance(item,dict): text=str(item.get("text") or item.get("snippet") or item.get("evidence") or item); source=item.get("source") or item.get("file") or item.get("path")
            else: text=str(item); source=None
            refs[ref]={"ref":ref,"text":text[:5000],"source":source}; numbered.append({"ref":ref,"text":text[:2400],"source":source})
        gates[gate]=numbered
    return refs,gates


def compact_review_evidence(row: dict, per_gate: int=3, chars: int=1400) -> dict:
    snippets=row.get("gate_snippets") or {}; packed={}
    for gate in REQUIRED_GATES:
        out=[]
        for item in list(snippets.get(gate) or [])[:per_gate]:
            if isinstance(item,dict): text=str(item.get("text") or item.get("snippet") or item.get("evidence") or ""); source=item.get("source") or item.get("file") or item.get("path")
            else: text=str(item or ""); source=None
            text=" ".join(text.split())[:chars]
            if text: out.append({"text":text,"source":source})
        packed[gate]=out
    return packed


def refresh_authority(row: dict) -> dict:
    if not row.get("gate_readiness"): return row
    rel=str(row.get("artifact_relative_root") or "").strip()
    if not rel: return row
    base=Path(os.getenv("DCE_FAST_ROOT","out"))/rel
    if not (base/"manifest.json").exists(): return row
    try: authority=reconcile_authority(base)
    except Exception as exc: row["authority_refresh_error"]=str(exc)[:500]; return row
    row["authority_conflicts"]=authority; deadline=authority.get("deadline") if isinstance(authority,dict) else None
    if isinstance(deadline,dict): row["deadline_authority_status"]=deadline.get("status"); row["deadline_conflict"]=bool(deadline.get("conflict"))
    return row
...
def call_model(row: dict, model: str, api_key: str, retries: int=2):
    ...
    payload={"candidate":{k:row.get(k) for k in ("candidate_id","title","buyer","portal","deadline","estimated_value","currency","notice_url","content_quality","gate_readiness")},"authority_conflicts":row.get("authority_conflicts"),"mandatory_gates":REQUIRED_GATES,"evidence_refs_by_gate":gates,"required_output":{"classification":"one of FINAL_SUPER_GREEN/GREEN/GREEN_PARTNERABLE/YELLOW/RED/MODEL_REVIEW_REQUIRED","score":"integer 0..100","summary":"short operational verdict","gates":{gate:{"status":"...","evidence_refs":[],"notes":"..."} for gate in REQUIRED_GATES}}}
    ...
    for attempt in range(1,max(0,retries)+2):
        try:
            ...
            if r.status_code>=400:
                ...
                if r.status_code not in RETRYABLE_HTTP or attempt>retries: raise last_error
            else:
                text=output_text(r.json()).strip()
                if not text: raise RuntimeError("OpenAI response contained no output_text")
                text=re.sub(r"^```(?:json)?\s*|\s*```$","",text,flags=re.I|re.S).strip(); obj=json.loads(text)
                if not isinstance(obj,dict): raise RuntimeError("model output was not a JSON object")
                return obj,refs
        except (requests.RequestException,json.JSONDecodeError,RuntimeError) as exc:
            last_error=exc
            if attempt>retries: raise
        try: retry_after=float(r.headers.get("retry-after") or 0) if 'r' in locals() else 0
...
def fallback_record(row: dict, reason: str, model: str|None=None):
    ...
    return {"candidate_id":row.get("candidate_id"),"title":row.get("title"),"buyer":row.get("buyer"),"portal":row.get("portal"),"notice_url":row.get("notice_url"),"deadline":row.get("deadline"),"estimated_value":row.get("estimated_value"),"currency":row.get("currency"),"preliminary_score":row.get("preliminary_score"),"classification":"MODEL_REVIEW_REQUIRED","final_score":prelim,"summary":reason,"model":model,"model_review_completed":False,"content_quality":row.get("content_quality"),"gate_readiness":bool(row.get("gate_readiness")),"evidence_quality":row.get("evidence_quality") or {},"authority_conflicts":row.get("authority_conflicts") or {},"gate_evidence_candidates":compact_review_evidence(row),"gates":{gate:{"status":"UNKNOWN","evidence":[],"notes":"Awaiting model/manual adjudication; absence of evidence is not a PASS."} for gate in REQUIRED_GATES}}
...
def normalize_model_record(row: dict, obj: dict, refs: dict, model: str):
    ...
    for gate in REQUIRED_GATES:
        item=raw_gates.get(gate) if isinstance(raw_gates,dict) else None; item=item if isinstance(item,dict) else {}; status=str(item.get("status") or "UNKNOWN").upper()
        if status not in ALLOWED_GATE_STATUSES: status="UNKNOWN"
        requested_refs=item.get("evidence_refs") or []; requested_refs=requested_refs if isinstance(requested_refs,list) else []; valid=[refs[x] for x in requested_refs if isinstance(x,str) and x in refs and x.startswith(gate+":")]; notes=str(item.get("notes") or "")[:3000]
        if status in {"PASS","PASS_CONDITIONAL"} and not valid: status="UNKNOWN"; notes=(notes+" | Downgraded: no valid evidence ref supplied.").strip(" |").strip()
        if status=="NOT_APPLICABLE" and not notes: status="UNKNOWN"; notes="Downgraded: NOT_APPLICABLE lacked a reason."
    ...
    if violations:
        ...
        if rec["classification"] in FINAL_CLASSIFICATIONS or rec["final_score"]>=90: rec["classification"]="MODEL_REVIEW_REQUIRED"; rec["final_score"]=min(89,rec["final_score"]); rec["gate_evidence_candidates"]=compact_review_evidence(row); rec["summary"]=(rec["summary"]+" | Finalization blocked by deterministic guard.").strip()
    return rec


def adjudicate_one(index,row,model,api_key,retries):
    try: obj,refs=call_model(row,model,api_key,retries=retries); return index,normalize_model_record(row,obj,refs,model),False
    except Exception as exc: return index,fallback_record(row,f"Model adjudication error: {str(exc)[:700]}",model),True
...
def main():
    ...
    rows=[refresh_authority(r) for r in load_jsonl(Path(args.queue))]; out=Path(args.out); out.mkdir(parents=True,exist_ok=True); api_key=os.getenv("OPENAI_API_KEY","").strip(); model=os.getenv("OPENAI_ADJUDICATION_MODEL","gpt-5-mini").strip() or "gpt-5-mini"; records=[None]*len(rows); review_jobs=[]; gate_ready_rows=[r for r in rows if r.get("gate_readiness")]; review_budget=max(0,args.max_model_reviews)
    for idx,row in enumerate(rows):
        if not row.get("gate_readiness"): rec=fallback_record(row,"Not gate-ready: authoritative DCE evidence is incomplete or unverified."); rec["classification"]="YELLOW"; records[idx]=rec; continue
        if not api_key: records[idx]=fallback_record(row,"OPENAI_API_KEY unavailable: published to ChatGPT-ready hot review bank.",model); continue
        if len(review_jobs)>=review_budget: records[idx]=fallback_record(row,"Per-run model review cap reached; published to ChatGPT-ready hot review bank.",model); continue
    ...
    if review_jobs:
        with ThreadPoolExecutor(max_workers=workers) as pool:
            ...
            for fut in as_completed(futures): idx,rec,errored=fut.result(); records[idx]=rec; model_errors+=int(errored)
    ...
    (out/"supergreen_shortlist.json").write_text(json.dumps({"dce_candidates":len(rows),"gate_ready":len(gate_ready_rows),"final_supergreen":len(finals),"green_or_partnerable":len(greens),"items":greens},indent=2,ensure_ascii=False),encoding="utf-8"); (out/"review_required.json").write_text(json.dumps(review_required[:120],indent=2,ensure_ascii=False),encoding="utf-8")
    summary={"candidates":len(rows),"gate_ready":len(gate_ready_rows),"model_available":bool(api_key),"model":model if api_key else None,"model_attempts":len(review_jobs),"model_errors":model_errors,"model_concurrency":workers if review_jobs else 0,"classification_counts":dict(counts),"final_supergreen":len(finals),"green_or_partnerable":len(greens),"review_required":len(review_required),"chatgpt_hot_review_ready":sum(1 for r in review_required if r.get("gate_readiness") and r.get("gate_evidence_candidates")),"guard_contract":"FINAL_SUPER_GREEN/90+ is accepted only after final_verdict_guard.py returns no violations."}; (out/"summary.json").write_text(json.dumps(summary,indent=2,ensure_ascii=False),encoding="utf-8"); print(json.dumps(summary,indent=2,ensure_ascii=False))
