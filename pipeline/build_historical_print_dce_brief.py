...
PATTERNS = {
    ...
    "delivery_logistics": [
        r"(?:livraison|expédition|routage|mise sous pli|conditionnement|stockage).{0,200}",
    ],
    "finishing": [
        r"(?:façonnage|agraf|pellicul|reliure|pliage|massicot|dos carré|encoll).{0,180}",
    ],
    "source_files_ip": [
        r"(?:fichiers? sources?|propriété intellectuelle|droits? d['’]auteur|cession des droits).{0,180}",
    ],
}

PRICE_RX = re.compile(r"(?:prix|coût).{0,70}(\d{1,3}(?:[,.]\d+)?)\s*%", re.I | re.S)
TECH_RX = re.compile(r"(?:valeur technique|technique|qualit[eé]).{0,70}(\d{1,3}(?:[,.]\d+)?)\s*%", re.I | re.S)
...
def contexts(text: str, regexes: list[str], max_hits=4, before=150, after=260):
    ...
    for pat in regexes:
        for m in re.finditer(pat,text,re.I|re.S):
            ...
            seen.add(key)
            out.append({"match":clean(m.group(0))[:200],"context":c[:650]})
...
def scalar_hits(text: str):
    def vals(rx):
        return [m.group(1) for m in list(rx.finditer(text))[:5]]
    return {
        "price_weight_pct_candidates": vals(PRICE_RX),
        "technical_or_quality_weight_pct_candidates": vals(TECH_RX),
...
def main():
    ...
    for d,c,m,e,text in iter_candidates(Path(a.root)):
        ...
        rows.append({
            "candidate_id":c.get("candidate_id"),"sub_niche":c.get("historical_sub_niche"),"buyer":c.get("buyer"),"title":c.get("title"),
            "content_quality":e.get("content_quality"),
            "files":[{"name":x.get("name"),"source_url":x.get("source_url"),"size":x.get("size")} for x in (m.get("files") or [])],
            "signals":sig,"scalars":scalar_hits(text),
        })
    ...
    summary={
        "contract":"HISTORICAL_PRINT_DCE_BUSINESS_BRIEF_V1","true_gate_ready_print_dces":len(rows),"final_verdict_allowed":False,
        "warning":"Strict evidence extraction from tiny historical sample. Empty signal = UNKNOWN, not absent requirement.",
        "signal_candidate_counts":{k:sum(bool(r['signals'][k]) for r in rows) for k in PATTERNS},
        "subniches":dict(Counter(str(r.get('sub_niche')) for r in rows)),
    }
    ...
    for r in rows:
        ...
        found=[k for k,v in r['signals'].items() if v]
        lines.append("Signals: "+(", ".join(found) if found else "none detected by strict patterns"))
