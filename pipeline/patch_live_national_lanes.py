...
# Add SI/SK to the source matrix without touching the ePPS matrix above.
m=re.search(r'(\n\s*source:\s*\[)([^\]]+)(\])',tail)
if not m: raise SystemExit('global source matrix missing')
items=[x.strip() for x in m.group(2).split(',') if x.strip()]
for source in ('SI_EJN','SK_UVO'):
...
# Only inject in global-official segment and once.
...
if text==original:
    ...
    print('Patched SuperGreen Discovery V2 with SI_EJN + SK_UVO')
