...
def competition_type(ntype):
    n=(ntype or '').lower()
    hard_no=('concluded','contract-change','contract_mod','award','result','completion','implementation','planning','prior-information','ex-ante','cancel')
    if any(x in n for x in hard_no):return False
    # The daily IUB corpus mixes notices, contracts and execution records. Competition publications carry notice/competition/tender semantics.
    return any(x in n for x in ('notice','competition','tender','call'))
def main():
    ...
    for ago in range(DAYS):
        ...
        for o in iter_notices(data):
            ...
            proceeding=next((x for x in urls if x not in docs and ('eis.gov.lv' in x or 'iub.gov.lv' in x)),None)
    ...
    stats={'source':'LV_IUB','raw_materialized':len(rows),'current_materialized':len(cur),'competition_materialized':sum(x['competition_notice'] for x in rows),'lookback_days':DAYS,'generated_at':NOW.isoformat(),'errors':[],'official_url':BASE,'telemetry':tele,'notice_type_counts':dict(types.most_common()),'archive_available_from':'2013-01-01'}
