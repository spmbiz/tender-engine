...
def extract_dates(text: str) -> list[dict]:
    ...
    for m in DATE_PATTERNS[0].finditer(text):
        y,mo,d=map(int,m.groups()); key=_valid_date(y,mo,d)
        if key and key not in seen: seen.add(key); rows.append({'date':key,'match':m.group(0),'context':text[max(0,m.start()-220):m.end()+220]})
    for m in DATE_PATTERNS[1].finditer(text):
        d,mo,y=map(int,m.groups()); key=_valid_date(y,mo,d)
        if key and key not in seen: seen.add(key); rows.append({'date':key,'match':m.group(0),'context':text[max(0,m.start()-220):m.end()+220]})
    for idx in (2,3):
...
def process(root: Path) -> dict:
    ...
    context,stats=deadline_contexts(gates,corpus) if evidence.get('gate_readiness') else ('',{'gate_contexts':0,'corpus_contexts':0,'unique_contexts':0}); found=extract_dates(context); dce_dates=sorted({x['date'] for x in found})
    if not evidence.get('gate_readiness'): status='NOT_APPLICABLE_DCE_NOT_GATE_READY'; conflict=False
    elif not dce_dates: status='UNKNOWN_NO_DCE_DEADLINE_PARSED'; conflict=False
    elif notice_date and notice_date in dce_dates: status='CONSISTENT_NOTICE_DATE_FOUND_IN_DCE'; conflict=False
    elif notice_date: status='DEADLINE_CONFLICT_REVIEW_REQUIRED'; conflict=True
    elif len(dce_dates)==1: status='DCE_DEADLINE_FOUND_NOTICE_DEADLINE_MISSING'; conflict=False
    else: status='DCE_MULTIPLE_DEADLINE_DATES_REVIEW_REQUIRED'; conflict=True
    result={'contract':'AUTHORITY_CONFLICTS_V2','candidate_id':candidate.get('candidate_id') or root.name,'deadline':{'status':status,'conflict':conflict,'notice_deadline_raw':candidate.get('deadline'),'notice_deadline_date':notice_date,'dce_deadline_candidates':dce_dates,'evidence':found[:30],'context_stats':stats,'rule':'Never silently overwrite deadline metadata. Deadline candidates come only from authoritative gate-ready DCE text near deadline language; unresolved/multiple/conflicting dates require review before final 90+/FINAL_SUPER_GREEN.'}}
    (root/'authority_conflicts.json').write_text(json.dumps(result,indent=2,ensure_ascii=False),encoding='utf-8'); return result


def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--root',default='out'); args=ap.parse_args(); roots=sorted(set(p.parent for p in Path(args.root).rglob('manifest.json'))); rows=[process(r) for r in roots]; statuses=sorted({r['deadline']['status'] for r in rows}); print(json.dumps({'candidates':len(rows),'deadline_conflicts':sum(1 for r in rows if r['deadline']['conflict']),'deadline_parsed':sum(1 for r in rows if r['deadline']['status'] not in {'UNKNOWN_NO_DCE_DEADLINE_PARSED','NOT_APPLICABLE_DCE_NOT_GATE_READY'}),'statuses':{s:sum(1 for r in rows if r['deadline']['status']==s) for s in statuses}},indent=2,ensure_ascii=False))
