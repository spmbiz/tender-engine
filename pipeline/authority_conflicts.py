...
def extract_dates(text: str) -> list[dict]:
    ...
    for m in DATE_PATTERNS[0].finditer(text):
        ...
        if key and key not in seen:
            seen.add(key); rows.append({'date': key, 'match': m.group(0), 'context': text[max(0, m.start()-220):m.end()+220]})
...
def deadline_contexts(gates: dict, corpus: str) -> tuple[str, dict]:
    ...
    for hit in hits:
        if isinstance(hit, dict):
            gate_chunks.extend(_contexts_around_deadline_words(str(hit.get('snippet') or ''), before=500, after=900, max_hits=20))
    ...
    for chunk in gate_chunks + corpus_chunks:
        ...
        if key not in seen:
            seen.add(key); all_chunks.append(chunk)
