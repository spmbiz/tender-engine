...
try:
    import fcntl  # Linux/GitHub Actions; optional elsewhere.
except Exception:  # pragma: no cover - Windows/local compatibility
    fcntl = None
...
def _write_cache(url: str, body: bytes, resolved_url: str) -> None:
    payload_path, meta_path, _ = _cache_paths(url)
    # Validate before persistence so a gateway/error page can never poison the
    ...
    meta_tmp.write_text(
        json.dumps({"source_url": url, "resolved_url": resolved_url, "cached_at": time.time()}),
        encoding="utf-8",
    )
...
def _fetch_atom_cached(
    session: requests.Session, url: str, read_timeout: int
) -> tuple[bytes, str, int, bool]:
    cached = _read_cache(url)
    if cached:
        return cached[0], cached[1], 200, True

    # Multiple ES candidates inside one HTTP shard are separate worker processes.
    # The file lock ensures only one of them downloads each 10-16 MB Atom page;
    # the others reuse the same validated bytes as soon as the first write lands.
    with _url_lock(url):
        cached = _read_cache(url)
        if cached:
            return cached[0], cached[1], 200, True
        ...
        if response.ok and body:
            try:
                ...
                # Cache failure must never alter resolver correctness.
                pass
...
def _resolve_from_atom_cached(candidate: dict, manifest: dict) -> tuple[list[str], bool, str]:
    ...
    session.headers.update(
        {
            "User-Agent": "Tender-Engine/17.0 public procurement research",
    ...
    for lane, start_url in v15._feed_order(candidate):
        ...
        while (
            url
            and url not in seen
            and page_no < v15.PLACSP_ATOM_MAX_PAGES
            and time.monotonic() - started < v15.PLACSP_ATOM_BUDGET
        ):
            seen.add(url)
            page_no += 1
            ...
            try:
                body, resolved_url, status, cache_hit = _fetch_atom_cached(
                    session, url, read_timeout
                )
                row = {
                    "lane": lane,
                    "page": page_no,
...
def adapter_placsp_v17(candidate: dict, out: Path, manifest: dict) -> None:
    ...
    advertised = resolution.get("document_urls") or []
    if not advertised:
        return

    # If the authoritative Atom entry advertises DCE but its document endpoints
    # temporarily return 5xx/429/network errors, NO_PUBLIC_FILE is semantically
    # wrong. Keep the candidate retryable rather than hiding a real DCE route.
    attempts = (manifest.get("dce_method_attempts") or [])[before:]
    document_attempts = [
        a
        for a in attempts
        if isinstance(a, dict)
        and str(a.get("method") or "").startswith("ES_PLACSP_")
        and "DOCUMENT" in str(a.get("method") or "")
    ]
    retryable = any(
        str(a.get("outcome") or "") == "ERROR"
        or int(a.get("http_status") or 0) == 429
        or int(a.get("http_status") or 0) >= 500
...
# v15's adapter resolves this global symbol at call time, so replacing it keeps
# all v15 behavior intact while adding process-shared Atom caching.
