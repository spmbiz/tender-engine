...
def main() -> None:
    ...
    ap.add_argument("--max-pages", type=int, default=int(os.getenv("LT_CVP_MAX_PAGES", "20")))
    args = ap.parse_args()
    ...
    for page_num in range(1, max_pages + 1):
        ...
        for row in rows:
            ...
            records[normalized["candidate_id"]] = normalized
    ...
    stats = {
        ...
        "max_pages": max_pages,
        "exhausted": exhausted,
        "truncated_by_page_cap": bool(not exhausted and len(telemetry) >= max_pages and not errors),
        "invalid_source_dates_observed": invalid_dates,
        "errors": errors,
        "telemetry": telemetry,
        "generated_at": now.isoformat(),
        "source_url": ENDPOINT,
        "semantics": "Currentness is fail-closed and requires a valid future tenderSubmissionDeadline. Malformed/missing dates never promote a record to current.",
    }
