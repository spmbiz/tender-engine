...
def main():
    ...
    pw=browser=context=page=None;net=[];console=[];anchors=[];buttons=[];body=''
    try:
        ...
        page.on('response',resp);page.on('console',lambda m: console.append({'type':m.type,'text':m.text[:500]}))
        page.goto(BASE,wait_until='domcontentloaded',timeout=60000)
        try:page.wait_for_load_state('networkidle',timeout=18000)
        except Exception:page.wait_for_timeout(5000)
        body=page.locator('body').inner_text(timeout=10000)
        anchors=page.eval_on_selector_all('a[href]','els => els.map(a => ({href:a.href,text:(a.innerText||a.textContent||"").trim()}))') or []
        buttons=page.eval_on_selector_all('button','els => els.map(b => ({text:(b.innerText||b.textContent||"").trim(),aria:b.getAttribute("aria-label")}))') or []
        # Explore only obvious public publication/search links; no login/registration interaction.
        candidate=[]
        for a in anchors:
            text=str(a.get('text') or '').lower();href=str(a.get('href') or '')
            if href.startswith('http') and any(k in (text+' '+href.lower()) for k in ('publication','publicatie','publication','notice','bekendmaking','avis','search','recherche','zoeken','bulletin')):candidate.append(href)
        for href in list(dict.fromkeys(candidate))[:8]:
            try:
                ...
                hs=page.eval_on_selector_all('a[href]','els => els.slice(0,120).map(a => ({href:a.href,text:(a.innerText||a.textContent||"").trim()}))') or []
    ...
    # Deduplicate network records while retaining first-seen order.
    seen=set();network=[]
    for x in net:
        key=(x.get('method'),x.get('url'),x.get('status'))
        if key not in seen:seen.add(key);network.append(x)
    probe={'source':'BE_EPROC_PUBLIC','generated_at':NOW.isoformat(),'base':BASE,'body_preview':body[:15000],'anchors':anchors[:300],'buttons':buttons[:150],'network':network[:600],'console':console[:100]}
    (OUT/'probe.json').write_text(json.dumps(probe,ensure_ascii=False,indent=2),encoding='utf-8')
    stats={'source':'BE_EPROC_PUBLIC','generated_at':NOW.isoformat(),'raw_materialized':0,'current_materialized':0,'probe_only':True,'anchor_count':len(anchors),'network_count':len(network),'api_like_count':sum('/api/' in str(x.get('url','')).lower() for x in network),'errors':[],'official_url':BASE}
    (OUT/'stats.json').write_text(json.dumps(stats,indent=2),encoding='utf-8');print(json.dumps(stats,indent=2));print('BODY',body[:3000]);print('NETWORK',json.dumps(network[:120],ensure_ascii=False,indent=2));print('ANCHORS',json.dumps(anchors[:120],ensure_ascii=False,indent=2))
