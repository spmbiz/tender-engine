...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        title=await page.title(); telemetry["page_title"]=title
        html=await page.content(); telemetry["html_bytes"]=len(html)
        if resp and resp.status>=400:
            raise RuntimeError(f"AusTender public listing returned HTTP {resp.status}")
        rows=page.locator("tr")
        n=await rows.count(); telemetry["table_rows_seen"]=n
        for i in range(n):
            tr=rows.nth(i); cells=tr.locator("td"); cc=await cells.count()
            if cc<3: continue
            vals=[clean(await cells.nth(j).inner_text()) for j in range(cc)]
            links=tr.locator("a[href]"); lc=await links.count()
            href=None; text_link=None
            for j in range(lc):
                ...
                if h and ("ATM" in h.upper() or "ATM" in t.upper() or "view" in h.lower()):
                    href=h; text_link=t; break
            if not href and lc:
                href=await links.nth(0).get_attribute("href"); text_link=clean(await links.nth(0).inner_text())
            # Identify ATM id from row/link; keep only genuine opportunity rows.
            joined=" | ".join(vals)
            m=re.search(r"\b(?:ATM\s*)?([A-Z0-9][A-Z0-9._/-]{4,})\b", text_link or "")
            atm=(m.group(1) if m else clean(vals[0]))
            if not href or not atm or len(joined)<20: continue
            # AusTender table layouts can evolve. Detect dates and choose title as the longest non-date text cell.
            datevals=[v for v in vals if re.search(r"\b20\d{2}\b",v)]
            deadline=None
            for dv in reversed(datevals):
                x=pdt(dv)
    ...
    stats={"source":"AU_AUSTENDER","raw_materialized":len(rows),"current_materialized":len(cur),"generated_at":NOW.isoformat(),"errors":[],"official_url":URL,"telemetry":telemetry}
