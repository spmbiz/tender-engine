...
def save_bytes(url,body,ct=''):
    ...
    fp=OUT/name; fp.write_bytes(body)
...
with sync_playwright() as p:
    ...
    ctx=b.new_context(accept_downloads=True,user_agent='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/151 Safari/537.36')
    page=ctx.new_page()
    try:
        ...
        # Direct browser-context HTTP fetch is deterministic for public WordPress assets.
        for x in likely[:30]:
            href=urllib.parse.urljoin(page.url,x['href'])
            try:
                resp=ctx.request.get(href,timeout=30000,fail_on_status_code=False)
                ct=(resp.headers.get('content-type') or '').lower()
                if resp.ok and (re.search(r'\.(pdf|docx?|xlsx?|zip)(?:\?|$)',href,re.I) or any(t in ct for t in ['pdf','zip','officedocument','msword','excel'])):
                    save_bytes(href,resp.body(),ct)
